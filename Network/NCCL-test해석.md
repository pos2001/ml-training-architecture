...
### Ring All-Reduce는 사용자 데이터보다 약 2배 많은 데이터를 네트워크로 전송합니다
```
Busbw = Algbw × 2(N-1)/N
...
### Busbw = 실제 속도 (중요!)
```
Algbw는 알고리즘과 노드 수 N에 따라 값이 변합니다. 그래서 하드웨어 효율 지표로 못 씁니다.
Busbw는 통신 패턴을 정규화해서 노드 수가 늘어도 대체로 일정하게 유지되도록 만든 값입니다. 이게 바로 NIC/하드웨어 실측 대역폭과 비교하는 진짜 지표입니다. nccl-tests를 busbw 기준으로 읽으라고 하는 이유가 이겁니다.
