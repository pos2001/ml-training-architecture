...
Scheduler: slurm
SlurmSettings:
ScaledownIdletime: 1440  # ODCR이므로 길게
SlurmQueues:
- Name: teamA
ComputeResources:
- Name: p5
...
CapacityReservationId: cr-xxxxx  # 같은 ODCR
Networking:
SubnetIds:
- subnet-xxxxx
...
결과:
- ParallelCluster가 각 큐를 독립적으로 관리
- 각 큐는 Slurm 파티션으로 자동 변환
- config.yaml로 선언적 관리
...
결과:
- ParallelCluster는 하나의 큐로 인식
- Slurm 레벨에서만 파티션 분리
- slurm.conf 수동 관리

pcluster update-cluster 실행 시:

1. config.yaml 읽기
2. slurm.conf 완전히 재생성
3. 기존 slurm.conf 덮어쓰기
4. 수동 추가한 내용 모두 삭제됨 ❌
